import { MD3Theme } from '../types';

import md3BaselineTheme from './baseline';

export const md3GrayTheme: MD3Theme = {
  ...md3BaselineTheme,
  schemes: {
    light: {
      primary: '#006874',
      surfaceTint: '#006874',
      onPrimary: '#FFFFFF',
      primaryContainer: '#9EEFFD',
      onPrimaryContainer: '#001F24',
      secondary: '#4A6267',
      onSecondary: '#FFFFFF',
      secondaryContainer: '#CDE7EC',
      onSecondaryContainer: '#051F23',
      tertiary: '#525E7D',
      onTertiary: '#FFFFFF',
      tertiaryContainer: '#DAE2FF',
      onTertiaryContainer: '#0E1B37',
      error: '#BA1A1A',
      onError: '#FFFFFF',
      errorContainer: '#FFDAD6',
      onErrorContainer: '#410002',
      background: '#F5FAFB',
      onBackground: '#171D1E',
      surface: '#F5FAFB',
      onSurface: '#171D1E',
      surfaceVariant: '#DBE4E6',
      onSurfaceVariant: '#3F484A',
      outline: '#6F797A',
      outlineVariant: '#BFC8CA',
      shadow: '#000000',
      scrim: '#000000',
      inverseSurface: '#2B3133',
      inverseOnSurface: '#ECF2F3',
      inversePrimary: '#82D3E0',
      primaryFixed: '#9EEFFD',
      onPrimaryFixed: '#001F24',
      primaryFixedDim: '#82D3E0',
      onPrimaryFixedVariant: '#004F58',
      secondaryFixed: '#CDE7EC',
      onSecondaryFixed: '#051F23',
      secondaryFixedDim: '#B1CBD0',
      onSecondaryFixedVariant: '#334B4F',
      tertiaryFixed: '#DAE2FF',
      onTertiaryFixed: '#0E1B37',
      tertiaryFixedDim: '#BAC6EA',
      onTertiaryFixedVariant: '#3B4664',
      surfaceDim: '#D5DBDC',
      surfaceBright: '#F5FAFB',
      surfaceContainerLowest: '#FFFFFF',
      surfaceContainerLow: '#EFF5F6',
      surfaceContainer: '#E9EFF0',
      surfaceContainerHigh: '#E3E9EA',
      surfaceContainerHighest: '#DEE3E5',
    },
    lightMediumContrast: {
      primary: '#004A53',
      surfaceTint: '#006874',
      onPrimary: '#FFFFFF',
      primaryContainer: '#25808C',
      onPrimaryContainer: '#FFFFFF',
      secondary: '#2F474B',
      onSecondary: '#FFFFFF',
      secondaryContainer: '#60797D',
      onSecondaryContainer: '#FFFFFF',
      tertiary: '#374260',
      onTertiary: '#FFFFFF',
      tertiaryContainer: '#687495',
      onTertiaryContainer: '#FFFFFF',
      error: '#8C0009',
      onError: '#FFFFFF',
      errorContainer: '#DA342E',
      onErrorContainer: '#FFFFFF',
      background: '#F5FAFB',
      onBackground: '#171D1E',
      surface: '#F5FAFB',
      onSurface: '#171D1E',
      surfaceVariant: '#DBE4E6',
      onSurfaceVariant: '#3B4446',
      outline: '#576162',
      outlineVariant: '#737C7E',
      shadow: '#000000',
      scrim: '#000000',
      inverseSurface: '#2B3133',
      inverseOnSurface: '#ECF2F3',
      inversePrimary: '#82D3E0',
      primaryFixed: '#25808C',
      onPrimaryFixed: '#FFFFFF',
      primaryFixedDim: '#006671',
      onPrimaryFixedVariant: '#FFFFFF',
      secondaryFixed: '#60797D',
      onSecondaryFixed: '#FFFFFF',
      secondaryFixedDim: '#486064',
      onSecondaryFixedVariant: '#FFFFFF',
      tertiaryFixed: '#687495',
      onTertiaryFixed: '#FFFFFF',
      tertiaryFixedDim: '#505B7B',
      onTertiaryFixedVariant: '#FFFFFF',
      surfaceDim: '#D5DBDC',
      surfaceBright: '#F5FAFB',
      surfaceContainerLowest: '#FFFFFF',
      surfaceContainerLow: '#EFF5F6',
      surfaceContainer: '#E9EFF0',
      surfaceContainerHigh: '#E3E9EA',
      surfaceContainerHighest: '#DEE3E5',
    },
    lightHighContrast: {
      primary: '#00272C',
      surfaceTint: '#006874',
      onPrimary: '#FFFFFF',
      primaryContainer: '#004A53',
      onPrimaryContainer: '#FFFFFF',
      secondary: '#0C2629',
      onSecondary: '#FFFFFF',
      secondaryContainer: '#2F474B',
      onSecondaryContainer: '#FFFFFF',
      tertiary: '#15213E',
      onTertiary: '#FFFFFF',
      tertiaryContainer: '#374260',
      onTertiaryContainer: '#FFFFFF',
      error: '#4E0002',
      onError: '#FFFFFF',
      errorContainer: '#8C0009',
      onErrorContainer: '#FFFFFF',
      background: '#F5FAFB',
      onBackground: '#171D1E',
      surface: '#F5FAFB',
      onSurface: '#000000',
      surfaceVariant: '#DBE4E6',
      onSurfaceVariant: '#1C2527',
      outline: '#3B4446',
      outlineVariant: '#3B4446',
      shadow: '#000000',
      scrim: '#000000',
      inverseSurface: '#2B3133',
      inverseOnSurface: '#FFFFFF',
      inversePrimary: '#BFF5FF',
      primaryFixed: '#004A53',
      onPrimaryFixed: '#FFFFFF',
      primaryFixedDim: '#003238',
      onPrimaryFixedVariant: '#FFFFFF',
      secondaryFixed: '#2F474B',
      onSecondaryFixed: '#FFFFFF',
      secondaryFixedDim: '#183034',
      onSecondaryFixedVariant: '#FFFFFF',
      tertiaryFixed: '#374260',
      onTertiaryFixed: '#FFFFFF',
      tertiaryFixedDim: '#202C49',
      onTertiaryFixedVariant: '#FFFFFF',
      surfaceDim: '#D5DBDC',
      surfaceBright: '#F5FAFB',
      surfaceContainerLowest: '#FFFFFF',
      surfaceContainerLow: '#EFF5F6',
      surfaceContainer: '#E9EFF0',
      surfaceContainerHigh: '#E3E9EA',
      surfaceContainerHighest: '#DEE3E5',
    },
    dark: {
      primary: '#82D3E0',
      surfaceTint: '#82D3E0',
      onPrimary: '#00363D',
      primaryContainer: '#004F58',
      onPrimaryContainer: '#9EEFFD',
      secondary: '#B1CBD0',
      onSecondary: '#1C3438',
      secondaryContainer: '#334B4F',
      onSecondaryContainer: '#CDE7EC',
      tertiary: '#BAC6EA',
      onTertiary: '#24304D',
      tertiaryContainer: '#3B4664',
      onTertiaryContainer: '#DAE2FF',
      error: '#FFB4AB',
      onError: '#690005',
      errorContainer: '#93000A',
      onErrorContainer: '#FFDAD6',
      background: '#0E1415',
      onBackground: '#DEE3E5',
      surface: '#0E1415',
      onSurface: '#DEE3E5',
      surfaceVariant: '#3F484A',
      onSurfaceVariant: '#BFC8CA',
      outline: '#899294',
      outlineVariant: '#3F484A',
      shadow: '#000000',
      scrim: '#000000',
      inverseSurface: '#DEE3E5',
      inverseOnSurface: '#2B3133',
      inversePrimary: '#006874',
      primaryFixed: '#9EEFFD',
      onPrimaryFixed: '#001F24',
      primaryFixedDim: '#82D3E0',
      onPrimaryFixedVariant: '#004F58',
      secondaryFixed: '#CDE7EC',
      onSecondaryFixed: '#051F23',
      secondaryFixedDim: '#B1CBD0',
      onSecondaryFixedVariant: '#334B4F',
      tertiaryFixed: '#DAE2FF',
      onTertiaryFixed: '#0E1B37',
      tertiaryFixedDim: '#BAC6EA',
      onTertiaryFixedVariant: '#3B4664',
      surfaceDim: '#0E1415',
      surfaceBright: '#343A3B',
      surfaceContainerLowest: '#090F10',
      surfaceContainerLow: '#171D1E',
      surfaceContainer: '#1B2122',
      surfaceContainerHigh: '#252B2C',
      surfaceContainerHighest: '#303637',
    },
    darkMediumContrast: {
      primary: '#86D7E5',
      surfaceTint: '#82D3E0',
      onPrimary: '#001A1D',
      primaryContainer: '#499CA9',
      onPrimaryContainer: '#000000',
      secondary: '#B5CFD4',
      onSecondary: '#011A1D',
      secondaryContainer: '#7C959A',
      onSecondaryContainer: '#000000',
      tertiary: '#BECAEF',
      onTertiary: '#081531',
      tertiaryContainer: '#8490B2',
      onTertiaryContainer: '#000000',
      error: '#FFBAB1',
      onError: '#370001',
      errorContainer: '#FF5449',
      onErrorContainer: '#000000',
      background: '#0E1415',
      onBackground: '#DEE3E5',
      surface: '#0E1415',
      onSurface: '#F6FCFD',
      surfaceVariant: '#3F484A',
      onSurfaceVariant: '#C3CCCE',
      outline: '#9BA5A6',
      outlineVariant: '#7B8587',
      shadow: '#000000',
      scrim: '#000000',
      inverseSurface: '#DEE3E5',
      inverseOnSurface: '#252B2C',
      inversePrimary: '#005059',
      primaryFixed: '#9EEFFD',
      onPrimaryFixed: '#001417',
      primaryFixedDim: '#82D3E0',
      onPrimaryFixedVariant: '#003C44',
      secondaryFixed: '#CDE7EC',
      onSecondaryFixed: '#001417',
      secondaryFixedDim: '#B1CBD0',
      onSecondaryFixedVariant: '#223A3E',
      tertiaryFixed: '#DAE2FF',
      onTertiaryFixed: '#03102C',
      tertiaryFixedDim: '#BAC6EA',
      onTertiaryFixedVariant: '#2A3553',
      surfaceDim: '#0E1415',
      surfaceBright: '#343A3B',
      surfaceContainerLowest: '#090F10',
      surfaceContainerLow: '#171D1E',
      surfaceContainer: '#1B2122',
      surfaceContainerHigh: '#252B2C',
      surfaceContainerHighest: '#303637',
    },
    darkHighContrast: {
      primary: '#F1FDFF',
      surfaceTint: '#82D3E0',
      onPrimary: '#000000',
      primaryContainer: '#86D7E5',
      onPrimaryContainer: '#000000',
      secondary: '#F1FDFF',
      onSecondary: '#000000',
      secondaryContainer: '#B5CFD4',
      onSecondaryContainer: '#000000',
      tertiary: '#FCFAFF',
      onTertiary: '#000000',
      tertiaryContainer: '#BECAEF',
      onTertiaryContainer: '#000000',
      error: '#FFF9F9',
      onError: '#000000',
      errorContainer: '#FFBAB1',
      onErrorContainer: '#000000',
      background: '#0E1415',
      onBackground: '#DEE3E5',
      surface: '#0E1415',
      onSurface: '#FFFFFF',
      surfaceVariant: '#3F484A',
      onSurfaceVariant: '#F3FCFE',
      outline: '#C3CCCE',
      outlineVariant: '#C3CCCE',
      shadow: '#000000',
      scrim: '#000000',
      inverseSurface: '#DEE3E5',
      inverseOnSurface: '#000000',
      inversePrimary: '#002F35',
      primaryFixed: '#AAF3FF',
      onPrimaryFixed: '#000000',
      primaryFixedDim: '#86D7E5',
      onPrimaryFixedVariant: '#001A1D',
      secondaryFixed: '#D1ECF0',
      onSecondaryFixed: '#000000',
      secondaryFixedDim: '#B5CFD4',
      onSecondaryFixedVariant: '#011A1D',
      tertiaryFixed: '#E0E6FF',
      onTertiaryFixed: '#000000',
      tertiaryFixedDim: '#BECAEF',
      onTertiaryFixedVariant: '#081531',
      surfaceDim: '#0E1415',
      surfaceBright: '#343A3B',
      surfaceContainerLowest: '#090F10',
      surfaceContainerLow: '#171D1E',
      surfaceContainer: '#1B2122',
      surfaceContainerHigh: '#252B2C',
      surfaceContainerHighest: '#303637',
    },
  },
};

export default md3GrayTheme;
